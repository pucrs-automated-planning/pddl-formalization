...
Due: Thursday 8 September, 9h50am (final deadline Saturday 10th, with discounts)
...
For this assignment you will be using the open source implementation of Graphplan available with the [JavaGP] library. 
You can obtain a copy of this software from its Github project site, or you can download a more conveniently packaged version of the software from our Moodle area. 
Note that although we refer to [JavaGP], you are free to use any [other planning software](#other-planning-software) you find convenient (or interesting) to measure the performance of your formalisation. 
...
In this assignment you will formalise *RPG* to help a hero to get out of dungeon. 
...
-   One experimentation section where you measure the performance of [JavaGP] using your action formalisation for each of the domains, on multiple problems.
...
-   Domain Encoding (30%) — correctness of the domain encoding, in relation to the domain specification from Section [Overview](#overview);

-   Problem specifications (20%) — correctness of the problem specifications used for the experiments, particularly the initial state specification, as missing predicates here will jeopardise [JavaGP’s]
...
-   As a first step, you should at least look at the sample domain and problem files given at the beginning of this specification.
    Additionally, here is a link to [PDDL domain and problem files](http://planning.cis.strath.ac.uk/competition/domains.html) (for scenarios different from the one you need to model in this project), from past iterations of the International Planning Competition (IPC).
